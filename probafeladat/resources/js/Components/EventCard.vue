<template>
  ...
  >
    ...
    <div class="w-full h-40 mb-4">
      <img
        :src="event.image_path ? `/storage/${event.image_path}` : placeholder"
        alt="Esemény kép"
    ...
    <h3 class="text-md font-medium text-gray-600 mb-2">Dátum: {{ event.starts_at }}</h3>
    ...
    <!-- Szabad helyek -->
    <p class="font-medium mb-4" :class="event.is_full ? 'text-red-500' : 'text-green-600'">
      <span v-if="event.is_full">BETELT</span>
      <span v-else>{{ remainingSeats }} szabad hely</span>
    </p>
...
export default {
  ...
  methods: {
    ...
    signup() {
      if (!confirm("Biztosan jelentkezni szeretnél az eseményre?")) return;

      router.post(`/events/${this.event.id}/signup`, {}, {
        onSuccess: () => {
